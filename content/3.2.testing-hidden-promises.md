...
In everyday work, we often need to deal with Promises ([MDN article](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Using_promises)). If the function we test is `async` (it returns a `Promise`), we can use `async` and `await` in our tests (see "How to Test Promise-s Using async and await").
