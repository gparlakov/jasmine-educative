...
- ```js
  ...
  ```
  - it's an `async`-chronous method
  - it takes 2 parameters: `title` and `content` to create the article, and it invokes the `ArticleAPI`'s `create` method.
...
Let's create the 3 specs for the above logic:
